...
$(document).ready(function () {
    ...
    $('table#pos_table tbody').on('change', 'input.pos_quantity', function () {
        ...
        // tr.find('span.valor_setado').text(__currency_trans_from_en(line_total, true));

        $('.sub_total').val(__currency_trans_from_en(line_total, true));
        $('.valor_unitario').val(__currency_trans_from_en(unit_price_inc_tax, true));
        console.log(entered_qty)
        $('.quantidade').val(entered_qty);
    ...
    $('button#pos-save-pedido-print').click(function () {
        ...
        $('#pos-save-pedido').attr('disabled', 'disabled');
        
        if (sell_form.valid()) {
            window.onbeforeunload = null;
            sell_form.submit();
        } else {
            // Se o formulário não for válido, reabilita os botões
            $(this).removeAttr('disabled');
            $('#pos-save-pedido').removeAttr('disabled');
        }
    });
    ...
    $('button#pos-save-pedido').click(function () {
        // Desabilita o botão imediatamente após o clique
        $(this).attr('disabled', 'disabled');
        $('#pos-save-pedido-print').attr('disabled', 'disabled');
        ...
        if (sell_form.valid()) {
            ...
            // Se o formulário não for válido, reabilita os botões
            $(this).removeAttr('disabled');
            $('#pos-save-pedido-print').removeAttr('disabled');
        }
    });
                                ...
                                ({
                                    ...
                                    success: function (e) {
                                        ...
                                        if (e.status == 401) {
                                            ...
                                            $('#action').css('display', 'none')
                                            swal("Erro ao transmitir", e.responseJSON, "error").then(() => {
                                            ...
                                            try {
                                                ...
                                                swal("Erro ao transmitir", jsError.protNFe.infProt.xMotivo, "error").then(() => {
                                                    localredireciona(result.venda_id)
                                                })
                                            } catch {
    ...
    function habButtonPagMulti() {
        if ($('#disabled_payment_partial').val() == '1') {
            ...
            if (bal_due == 0) {
                ...
                $('#pos-save').attr('disabled', 'disabled')
            }
            console.log("bal_due", bal_due)
        }
    }
    ...
    $(document).on('shown.bs.modal', '.row_edit__price_model', function () {
    ...
    if (add_via_ajax) {
        ...
        $.ajax({
            ...
            data: {
                ...
                quantidade_modal: $('#qtd_modal').val(),
            ...
            success: function (result) {
                ...
                if (result.success) {
                    ...
                    // dados do pdv reduzido
                    let qtd_modal = $('#qtd_modal').val()
                    $.get('/sells/pos/get_product_dados/' + variation_id + '/' + location_id)
                        .done((result) => {
                            // $('.codigo_barras').val(result.codigo_barras)
                            if (qtd_modal > 1) {
                                $('.quantidade').val(qtd_modal)
                                $('.sub_total').val(__currency_trans_from_en(result.default_sell_price * qtd_modal, true))
                            } else {
                                $('.quantidade').val(1)
                                $('.sub_total').val(__currency_trans_from_en(result.default_sell_price * 1, true))
                            }
                            $('.valor_unitario').val(__currency_trans_from_en(result.default_sell_price, true))
                        })
                        .fail((err) => {
                            console.log(err)
                        })
                    // fim dados do pdv reduzido
...
function pos_total_row() {
    ...
    var price_total = 0;
    let valor_frete = $('#valor_frete').val() ? parseFloat($('#valor_frete').val()) : 0
    ...
    $('table#pos_table tbody tr').each(function () {
        ...
        // $('.quantidade').val(__read_number($(this).find('input.pos_quantity')));
    });
...
function calculate_billing_details(price_total) {
    ...
    // ajuste para setar valor total a pagar quando for do tipo pendente em locacao
    $('#total_produto_add').val(__currency_trans_from_en(total_payable_rounded, false));
...
function calculate_balance_due() {
    ...
    // alert(total_paying)
    // teste para passar valor_recebido
    // let valor = $('.payment-amount').val()
    $('#valor_recebido').val(total_paying)
...
$('table#pos_table tbody').on('change', 'input.pos_line_total', function () {
    ...
    tr.find('span.valor_setado').text(__currency_trans_from_en(subtotal, true));
...
$(document).on('shown.bs.tab', 'a[href="#tab_sangria_suprimento"]', function () {
    get_recent_transactions('tab_sangria_suprimento', $('div#tab_sangria_suprimento'));
});
...
function gerarPorIntervalo(qtdParcelas, dataBase, intervalo, amount, call) {
    let map = [];
    let dia = dataBase.substring(0, 2)
    let mes = dataBase.substring(3, 5)
    let ano = dataBase.substring(6, 10)

...
function montaTabelaMunicipioCarregamento(){
	...
	MUNIPIOSCARREGAMENTO.map((v) => {
		...
		html += "<td>"+v.id+"</td>";
		html += "<td>"+v.nome+"</td>";
		html += "<td><a class='btn-trash' href='#!' onclick='deleteMunicipioCarregamento("+v.id+")'>"+
...
function validaInsertInfo(call){
	...
	if($('#chave_nfe').val().length > 0 && $('#seg_cod_nfe').val().length > 0){
		msg += "Informe somente o campo Chave NFe ou Chave NFe contigencia\n";
	}
	if($('#chave_cte').val().length > 0 && $('#seg_cod_cte').val().length > 0){
		msg += "Informe somente o campo Chave CTe ou Chave CTe contigencia\n";
	}

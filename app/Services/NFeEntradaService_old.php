...
class NFeEntradaService
{
	...
	public function transmitir($signXml, $chave, $cnpj)
	{
		try {
			...
			// ENVIO SÍNCRONO → adicionando parâmetros: $indSinc = 1, $compactar = false
			$resp = $this->tools->sefazEnviaLote([$signXml], $idLote, 1, false);

			$st = new \NFePHP\NFe\Common\Standardize();
			$std = $st->toStd($resp);

			// Não é mais necessário consultar recibo, já vem com protocolo
			if ($std->cStat != 104) {
				return "[$std->cStat] - $std->xMotivo";
			}
			...
			if ($std->protNFe->infProt->cStat != 100) {
				return "Rejeição: [{$std->protNFe->infProt->cStat}] - {$std->protNFe->infProt->xMotivo}";
			}

			try {
				// Aqui o protocolo vem direto como XML string, usamos diretamente o $resp (não $std)
				$xml = \NFePHP\NFe\Complements::toAuthorize($signXml, $resp);
				...
				return $std->protNFe->infProt->nRec ?? 'Recibo não informado';
			} catch (\Exception $e) {
				return [
					'erro' => true,

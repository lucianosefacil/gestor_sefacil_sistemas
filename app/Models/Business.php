...
class Business extends Model
{
    ...
    protected $appends = ['certificate_request_status'];


    public function getcertificateRequestStatusAttribute()
    {
        return RequestCertificate::where('request_certificates.business_id', $this->id)->orderBy('request_certificates.id', 'desc')->first()->status ?? false;
    }

    public function redirecionarPedido()
    {
        return json_decode(RequestCertificate::where('request_certificates.business_id', $this->id)->orderBy('request_certificates.id', 'desc')
            ->first()->response, true);
    }
    ...
    public function getPwdCertificado()
    {
        $pwd = base64_decode($this->senha_certificado);

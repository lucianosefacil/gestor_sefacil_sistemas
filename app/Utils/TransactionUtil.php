...
class TransactionUtil extends Util
{
    ...
    public function createSellTransaction($business_id, $input, $invoice_total, $user_id, $uf_data = true)
    {
        ...
        if (isset($input['is_locacao']) && $input['is_locacao'] == 1) {
            $last_locacao_invoice = \DB::table('transactions') 
                ->where('is_locacao', true) 
                ->where('business_id', $business_id)
                ->orderBy('invoice_no', 'desc')
                ->value('invoice_no'); 

            if ($last_locacao_invoice) {
                $invoice_no = str_pad((int)$last_locacao_invoice + 1, 3, '0', STR_PAD_LEFT);
            } else {
                $invoice_no = '001';
            }
        } else {
        ...
        // inserido essas duas variaves dia 03/06
        $vlRec = (float)str_replace(",", ".", $input['valor_recebido']);
        $vTotal = (float)str_replace(",", ".", $input['final_total']);
        ...
        $transaction = Transaction::create([
            ...
            // 'troco' => $input['valor_recebido'] > 0 ? ($input['valor_recebido'] - $final_total) : 0 ,
            'troco' => $input['valor_recebido'] > 0 ? $vlRec - $vTotal : 0,

            'valor_recebido' => $input['valor_recebido'] > 0 ? $input['valor_recebido'] : str_replace(",", ".", $final_total),
            'referencia_nfe' => isset($input['referencia_nfe']) ? $input['referencia_nfe'] : '',
            'is_locacao' => $input['is_locacao'] ?? 0,
        ]);
                            ...
                            if (!empty($product['modifier_sell_line_id'][$key])) {
                                ...
                                if (!empty($product['modifier_price'][$key])) {
                                    ...
                                    $modifiers_formatted[] = new TransactionSellLine([
                                        ...
                                        'n_pedido' => isset($product['n_pedido']) ? $product['n_pedido'] : null,
                                        'n_item' => isset($product['n_item']) ? $product['n_item'] : null,
                                    ]);
                                }
                            }
                        }
    ...
    public function createOrUpdatePaymentLines($transaction, $payments, $request, $business_id = null, $user_id = null, $uf_data = true)
    {
        ...
        $boletoData = isset($request->json_boleto) ? json_decode($request->json_boleto) : [];
    ...
    protected function insertRevenue($transaction, $vencimento, $valor, $user_id, $tipo_pagamento)
    {
        ...
        if ($transaction->is_locacao == true) {
            if ($tipo_pagamento == 'aguardando_pagamento' || $tipo_pagamento == 'boleto' || $tipo_pagamento == 'other') {
                $recebido = false;
            } else {
                $recebido = true;
            }
    ...
    public function getReceiptDetails($transaction_id, $location_id, $invoice_layout, $business_details, $location_details, $receipt_printer_type)
    {
        ...
        $output['is_locacao'] = $transaction->is_locacao;
        ...
        $output['cpf_cnpj'] = $customer->cpf_cnpj;
        ...
        } elseif ($transaction->status == 'pending' && $transaction->is_quotation == 1) {
        ...
        // teste para imprimir fatura no orcamento
            ...
            if ($il->show_payments == 1) {
                ...
                if (!empty($payments)) {
                    foreach ($payments as $value) {
                        ...
                        } elseif ($value['method'] == 'aguardando_pagamento') {
                            $output['payments'][] =
                                [
                                    'method' => $method . (!empty($value['transaction_no']) ? (', ' . trans("lang_v1.transaction_no") . ':' . $value['transaction_no']) : ''),
                                    'amount' => $this->num_f($value['amount'], $show_currency, $business_details),
    ...
    public function getOpenRevenues($business_id, $start_date = null, $end_date = null, $location_id = null, $created_by = null)
    {
        $query = DB::table('revenues')
            ->where('business_id', $business_id)
            ->where('status', 0)
            ->select(DB::raw('COALESCE(SUM(valor_total - valor_recebido), 0) as open_revenue'));
        ...
        $revenue_details = $query->first();

        return [
            'open_revenues' => $revenue_details->open_revenue ?? 0
        ];
    }

    public function getOpenExpenses($business_id, $start_date = null, $end_date = null, $location_id = null, $created_by = null)
    {
        $query = DB::table('transactions')
            ...
            // ->where('transactions.status', 'final')
            ->select(
                DB::raw('SUM(transactions.final_total) - IFNULL(SUM(transaction_payments.amount), 0) AS open_expenses')
            )
            ->groupBy('transactions.id');
        ...
        $output['open_expenses'] = $expense_details->sum('open_expenses') ?? 0;

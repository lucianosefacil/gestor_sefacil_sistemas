...
>>>>>>> 2a27942e30b693deaee5cc3077cd8eee7b8311c5
        return $count;
    }
    ...
    public function updateSellPayments($status_before, $transaction, $payments)
    {
        ...
        if ($status_before == 'draft' && $transaction->status == 'final') {
            ...
            ->select(
                ...
                DB::raw("SUM(IF(pay_method='pix', IF(type='credit', amount, -1 * amount), 0)) as total_pix"),
                DB::raw("SUM(IF(pay_method='debit', IF(type='credit', amount, -1 * amount), 0)) as total_debit"),
            )->first();
            if (!empty($prev_payments)) {
                $payment_diffs = [
                    ...
                    'pix' => $prev_payments->total_pix,
                    'debit' => $prev_payments->total_debit
                ];
    ...
    public function getRegisterDetails($register_id = null)
    {
        ...
        $register_details = $query->select(
            ...
            DB::raw("SUM(IF(pay_method='pix', IF(transaction_type='sell', amount, 0), 0)) as total_pix"),

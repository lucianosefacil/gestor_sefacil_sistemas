...
class ProductUtil extends Util
{
        ...
        foreach ($input_variations as $key => $value) {
            ...
            if (!empty($value['variations'])) {
                ...
                foreach ($value['variations'] as $k => $v) {
                    ...
                    $cod_barras = !empty($v['cod_barras']) ? $v['cod_barras'] : null;
    ...
    public function upProductQuantity($product_id, $variation_id, $location_id, $new_quantity, $old_quantity = 0)
    {
        $qty_difference = $new_quantity + $old_quantity;
    ...
    // teste de calculo servicos, quando houver somente servico na ordem de servico
    public function calculateInvoiceTotalServico($products, $tax_id, $discount = null)
    {
        if (empty($products)) {
            return false;
        }
        ...
        //Sub Total
        foreach ($products as $product) {
            $output['total_before_tax'] += $this->convert_num($product['valor_unitario']) * $this->num_uf($product['quantidade_servico']);
            // echo $this->num_uf($product['unit_price_inc_tax']);
            //Add modifier price to total if exists
            if (!empty($product['modifier_price'])) {
                foreach ($product['modifier_price'] as $modifier_price) {
    ...
    public function filterProduct($business_id, $search_term, $location_id = null, $not_for_selling = null, $price_group_id = null, $product_types = [], $search_fields = [], $check_qty = false)
    {
        ...
        $query->whereNotExists(function ($subquery) {
            $subquery->select(DB::raw(1))
                ->from('locacaos')
                ->whereColumn('locacaos.produto_id', 'products.id')
                ->where('locacaos.status', 'aberta');
        });
        ...
        if (!empty($search_term)) {
            ...
            $query->where(function ($query) use ($search_term, $search_fields) {
                ...
                // teste de busca por cod barras
                if (in_array('cod_barras', $search_fields)) {
                    $query->orWhere('cod_barras', 'like', '%' . $search_term . '%');
                }
        ...
        $query->select(
            ...
            'variations.sub_sku',
            'variations.cod_barras',
            // inserido cod_barras para busca
            'U.short_name as unit'
        );

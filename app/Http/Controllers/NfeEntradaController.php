...
class NfeEntradaController extends Controller
{
	public function novo($id)
	{
		...
		if ($purchase->numero_nfe > 0) {
			return redirect('/nfeEntrada/ver/' . $id)
				->with('status', [
					'success' => 0,
					'msg' => 'Já esta emitida NFe de entrada para esta compra!!'
				]);
		}
	...
	public function transmitir(Request $request)
	{
		...
		if ($transaction->estado == 'REJEITADO' || $transaction->estado == 'NOVO') {
			...
			try {
				...
				// (Opcional) Salvar o XML para debug
				\Storage::put('xml_debug_nfentrada.xml', $nfe['xml']);

				$resultado = $nfe_service_entrada->transmitir($signed, $nfe['chave'], $cnpj);

				$resArray = json_decode($resultado->getContent(), true);
				// dd($resultado);
				// Aqui o retorno é um array. Se não tiver erro explícito, consideramos sucesso.
				// if (is_array($resultado) && (!isset($resultado['erro']) || $resultado['erro'] == '')) {
				...
				if ($resArray['success'] === true) {
					...
					return response()->json([
						'success' => false,
						'mensagem' => $resArray['mensagem'] ?? 'Erro ao transmitir a NFe',
						'erro' => $resArray['mensagem'] ?? 'Erro não identificado',
						'tipo' => $resArray['tipo'] ?? 'erro'
					], 200);
				}
			} catch (\Exception $e) {
				...
				return response()->json([
					'success' => false,
					'mensagem' => 'Erro interno ao transmitir a NFe.',
					'erro' => $e->getMessage()
				], 200);
			}
		} else {
			return response()->json("Esta NFe já esta aprovada", 200);
		}
	}
	...
	public function imprimirCorrecao($id)
	{
		...
		try {
			if (file_exists(public_path('xml_nfe_entrada_correcao/' . $cnpj . '/' . $transaction->chave . '.xml'))) {

				$xml = file_get_contents(public_path('xml_nfe_entrada_correcao/' . $cnpj . '/' . $transaction->chave . '.xml'));

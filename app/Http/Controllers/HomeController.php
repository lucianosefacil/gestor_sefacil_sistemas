...
use App\Utils\ValidationsGenericRequestUtil;
...
class HomeController extends Controller
{
    ...
    {
        $isExpiredCertificate = ValidationsGenericRequestUtil::finishedCertificate(request());
        ...
        if ($isExpiredCertificate['alert']) {
            request()->session()->put('alert', ['message' => $isExpiredCertificate['message'], 'icon' => $isExpiredCertificate['icon'] ?? 'error', 'module' => $isExpiredCertificate['module']]);
        }
    ...
    public function getTotals()
    {
        if (request()->ajax()) {
            ...
            // parte de contas a receber no painel 
            $revenue_details = $this->transactionUtil->getOpenRevenues($business_id, $start, $end, $location_id);
            $expense_details = $this->transactionUtil->getOpenExpenses($business_id, $start, $end, $location_id);
            ...
            $transaction_totals = $this->transactionUtil->getTransactionTotals($business_id, $transaction_types, $start, $end, $location_id);
            ...
            // parte de contas a receber no painel
            $output['open_revenues'] = $revenue_details['open_revenues'] ?? 0;
            $output['open_expenses'] = $expense_details['open_expenses'] ?? 0;

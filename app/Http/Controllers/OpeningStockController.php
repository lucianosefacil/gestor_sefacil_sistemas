    ...
    public function add($product_id)
    {
        ...
        if (!empty($product) && $product->enable_stock == 1) {
            ...
            foreach ($transactions as $transaction) {
                // $purchase_lines = [];  retirado daqui e colocdo fora do foreach para teste, esta dando probela ao setar estoque dentro do produto.
        ...
        try {
                    ...
                    if (array_key_exists($location_id, $locations)) {
                        ...
                        foreach ($value as $k => $rows) {
                            foreach ($rows as $key => $v) {
                                // $v['purchase_price'] = str_replace(",", "", $v['purchase_price']);
                                // $v['quantity'] = str_replace(",", "", $v['quantity']);

                                $purchase_price = $this->productUtil->num_uf(($v['purchase_price']));
            ...
            if ($product->nuvemshop_id != '') {
                ...
                foreach ($opening_stocks as $key_os => $value) {
                    foreach ($value as $k => $rows) {
                        foreach ($rows as $key => $v) {
                           array_push($quantidades, $v['quantity']);    
                        }
                    }
                }
                for($i=0; $i<sizeof($quantidades); $i++){
                    $resp = $response = $api->put("products/$product->nuvemshop_id/variants/" . $produto->variants[$i]->id, [
                        'stock' => (int)str_replace(",", ".", $quantidades[$i])
                    ]);
                }
            }
            $output = [

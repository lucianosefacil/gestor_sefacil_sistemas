    ...
    */
    ...
    public function postRegister(Request $request)
    {
        ...
        try {
            ...
            //Create owner.
            $owner_details = $request->only(['surname', 'first_name', 'last_name', 'username', 'email', 'password', 'language', 'contador']);

            $owner_details['language'] = empty($owner_details['language']) ? config('app.locale') : $owner_details['language'];

            $user = User::create_user($owner_details);
            $business_details = $request->only(['name', 'start_date', 'currency_id', 'time_zone', 'tipo_contador', 'cnpj']);
            ...
            if ($package == null) {
                ...
                $business_details['enabled_modules'] = ['purchases', 'add_sale', 'pos_sale', 'stock_transfers', 'stock_adjustment', 'expenses', 'cte', 'mdfe', ''];
            } else {
    ...
    {
        if ($business->certificado == null) return null;
    ...
    public function postBusinessSettings(Request $request)
    {
        ...
        try {
            ...
            $business_details = $request->only([
                ...
                'rp_expiry_type', 'custom_labels', 'weighing_scale_setting', 'razao_social', 'cnpj', 'ie',
                ...
                'modelo_pdv', 'enable_tef_pdv', 'token_nfse', 'ultimo_numero_nfse', 'numero_serie_nfse', 'numero_rps', 'nome_ordem_servico'
            ]);

            $certificado = $request->file('certificado');
            $business_details['pCredSN'] = $business_details['pCredSN'] ?? 0;
            $business_details['enable_tef_pdv'] = $request->has('enable_tef_pdv') ? 1 : 0;

            if ($certificado) {

                $ctx = file_get_contents($certificado);
                $business_details['certificado'] = $ctx;
                $certificado_urn = Str::random(25) . "." . $certificado->getClientOriginalExtension();
                // $certificado_urn = $this->businessUtil->uploadFile($request, 'certificado', 'business_certificados', 'document');
                $upload = $certificado->move(public_path('uploads/business_certificados/'), $certificado_urn);
                if (!empty($certificado_urn)) {
                    $business_details['certificado_urn'] = $certificado_urn;
                }
            }
            ...
            $firstProduct = Product::where('business_id', $business_id)->first();

            if ($firstProduct != null) {
                if ($business_details['pCredSN'] != $firstProduct->pCredSN) {
                    Product::where('business_id', $business_id)
                        ->update(['pCredSN' => $business_details['pCredSN']]);
                }
            }
            ...
            $output = [
                'success' => 0,
                'msg' => __('messages.something_went_wrong') . ": " . $e->getLine()
            ];
        }
    ...
    public function novoparceiro()
    {
        ...
        return view('business/novo_parceiro', compact(
            'timezone_list',
            'months',
            'accounting_methods',
            'package_id',

...
use Closure;
use Menu;
use Nwidart\Menus\Facades\Menu as FacadesMenu;
use Nwidart\Menus\Menu as MenusMenu;
...
class AdminSidebarMenu
{
    ...
    public function handle($request, Closure $next)
    {
        ...
        Menu::create('admin-sidebar-menu', function ($menu) {
            ...
            if (auth()->user()->can('supplier.view') || auth()->user()->can('customer.view')) {
                ...
                $menu->dropdown(
                    ...
                    function ($sub) {
                        ...
                        //     '/contador',
                        //     'Contador',
                        //     ['icon' => 'fa fas fa-truck', 'active' => request()->segment(1) == 'contador' && request()->segment(2) == 'import']
                        // );
                    },
            ...
            // Ordem de Servico
            if (in_array('os', $enabled_modules) && (auth()->user()->can('os.view') || auth()->user()->can('os.create') || auth()->user()->can('os.update'))) {
                $menu->dropdown(
                    __('Ordem de Serviço'),
                    function ($sub) {
                        $sub->url(
                            action('OrdemServicoController@index'),
                            'Lista de OS',
                            ['icon' => 'os.view', 'active' => request()->segment(1) == 'os']
                        );

                        // $sub->url(
                        //     '/devolucao/lista',
                        //     'NFe de Devolução',
                        //     ['icon' => 'fa fas fa-exchange-alt', 'active' => request()->segment(1) == 'purchase-xml']
                        // );
                    ...
                    ['icon' => 'fa fas fa-wrench', 'id' => 'tour_step6']
                )->order(25);
            }
            // Fim OS
            ...
            // locacao
            if (in_array('locacao', $enabled_modules) && (auth()->user()->can('locacao.view') || auth()->user()->can('locacao.create') || auth()->user()->can('locacao.update'))) {
                $menu->dropdown(
                    __('Locação'),
                    function ($sub) {
                        $sub->url(
                            ...
                            'Locação',
                            ['icon' => 'locacao.view', 'active' => request()->segment(1) == 'locacao']
                        );
                    },
                    ['icon' => 'fa fas fa-list', 'id' => 'tour_step6']
                )->order(25);
            }
            // Fim locacao
                ...
                $menu->dropdown(
                    ...
                    function ($sub) use ($enabled_modules) {
                        ...
                        $sub->url(
                            '/nfelista',
                            'Lista de NFe',
                            ['icon' => 'fa fas fa-tasks', 'active' => request()->segment(1) == 'nfelista' && request()->segment(2) == null]
                        );

                        $sub->url(
                            '/nfcelista',
                            'Lista de NFCe',
                            ['icon' => 'fa fas fa-tasks', 'active' => request()->segment(1) == 'nfcelista' && request()->segment(2) == null]
                        );

                        $sub->url(
                            '/nfce-contigencia',
                            'Lista de Contigencia PDV',
                            ['icon' => 'fa fas fa-tasks', 'active' => request()->segment(1) == 'nfcelista' && request()->segment(2) == null]
                        );
                        ...
                        $sub->url(
                            '/import-xml',
                            'Importar XML',
                            ['icon' => 'fa fas fa-file', 'active' => request()->segment(1) == 'import-xml' && request()->segment(2) == null]
                        );
                    },
            ...
            //                 '/ecommerce/config',
            //                 'Configuração',
            //                 ['icon' => 'fa fas fa-cog', 'active' => request()->segment(1) == 'ecommerce' && request()->segment(2) == 'config']
            //             );

            //             $sub->url(
            //                 '/pedidosEcommerce',
            //                 'Pedidos',
            //                 ['icon' => 'fa fas fa-tags', 'active' => request()->segment(1) == 'pedidosEcommerce']
            //             );

            //             $sub->url(
            //                 '/clienteEcommerce',
            //                 'Clientes',
            //                 ['icon' => 'fa fas fa-user', 'active' => request()->segment(1) == 'clienteEcommerce' ]
            //             );

            //             $sub->url(
            //                 '/carrosselEcommerce',
            //                 'Carrossel Destaque',
            //                 ['icon' => 'fa fas fa-th', 'active' => request()->segment(1) == 'carrosselEcommerce']
            //             );

            //             $sub->url(
            //                 '/contatoEcommerce',
            //                 'Contato',
            //                 ['icon' => 'fa fas fa-comment', 'active' => request()->segment(1) == 'contatoEcommerce']
            //             );

            //             $sub->url(
            //                 '/informativoEcommerce',
            //                 'Informativo',
            //                 ['icon' => 'fa fas fa-bell', 'active' => request()->segment(1) == 'informativoEcommerce']
            //             );

            //             $sub->url(
            //                 '/freteGratis',
            //                 'Cidade Frete Grátis',
            //                 ['icon' => 'fa fas fa-city', 'active' => request()->segment(1) == 'freteGratis']
            //             );

            //             $sub->url(
            //                 '/cupom',
            //                 'Cupom de Desconto',
            //                 ['icon' => 'fa fas fa-tag', 'active' => request()->segment(1) == 'cupom']
            //             );
            ...
            // NUVEM SHOP
            if (in_array('nuvemshop', $enabled_modules) && auth()->user()->can('nuvemshop.view')) {
                ...
                $menu->dropdown(
                    ...
                    function ($sub) {
                        $sub->url(
                            '/nuvemshop/config',
                            'Configuração',
                            ['icon' => 'fa fas fa-cog', 'active' => request()->segment(1) == 'nuvemshop' && request()->segment(2) == 'config']
                        );

                        $sub->url(
                            '/nuvemshop/categorias',
                            'Categorias',
                            ['icon' => 'fa fas fa-tags', 'active' => request()->segment(1) == 'categorias']
                        );

                        $sub->url(
                            '/nuvemshop/pedidos',
                            'Pedidos',
                            ['icon' => 'fa fas fa-tags', 'active' => request()->segment(1) == 'pedidos']
                        );

                        $sub->url(
                            '/nuvemshop/clientes',
                            'Clientes',
                            ['icon' => 'fa fas fa-user', 'active' => request()->segment(1) == 'clientes']
                        );

                        $sub->url(
                            '/nuvemshop/produtos',
                            'Produtos',
                            ['icon' => 'fa fas fa-th', 'active' => request()->segment(1) == 'produtos']
                        );
            ...
            if (in_array('cte', $enabled_modules) && auth()->user()->can('cte.view')) {
                $menu->dropdown(
                    'CTe',
                    function ($sub) {
                        $sub->url(
                            action('CteController@index'),
                            'Listar',
                            ['icon' => 'fa fas fa-list', 'active' => request()->segment(1) == 'cte' && request()->segment(2) == null]
                        );

                        $sub->url(
                            action('CteController@create'),
                            'Adicionar',
                            ['icon' => 'fa fas fa-plus-circle', 'active' => request()->segment(1) == 'cte' && request()->segment(2) == 'create']
                        );
                        ...
                        $sub->url(
                            action('CteController@xmls'),
                            'Lista de XML',
                            ['icon' => 'fa fas fa-tasks', 'active' => request()->segment(1) == 'cte' && request()->segment(2) == 'xmls']
                        );
                    },
            ...
            //MDFe
            if (in_array('mdfe', $enabled_modules) && auth()->user()->can('mdfe.view')) {

                $menu->dropdown(
                    'MDFe',
                    function ($sub) {
                        $sub->url(
                            action('MdfeController@create'),
                            'Adicionar',
                            ['icon' => 'fa fas fa-list', 'active' => request()->segment(1) == 'mdfe' && request()->segment(2) == null]
                        );

                        $sub->url(
                            action('MdfeController@index'),
                            'Listar',
                            ['icon' => 'fa fas fa-plus-circle', 'active' => request()->segment(1) == 'mdfe' && request()->segment(2) == 'new']
                        );
                        ...
                        $sub->url(
                            action('MdfeController@xmls'),
                            'Lista de XML',
                            ['icon' => 'fa fas fa-tasks', 'active' => request()->segment(1) == 'mdfe' && request()->segment(2) == 'xmls']
                        );
                    },
                    ['icon' => 'fa fas fa-truck']
                )->order(33);
                ...
                $menu->dropdown(
                    ...
                    function ($sub) use ($enabled_modules) {
                        ...
                        if (auth()->user()->can('stock_report.view')) {
                            ...
                            $sub->url(
                                action('FiscalInventoryReportController@index'),
                                "Relatório Inv. Fiscal",
                        ...
                        if (auth()->user()->can('purchase_n_sell_report.view')) {
                            ...
                            $sub->url(
                                action('ExportarController@index'),
                                "Exportar para Excel",
                                ['icon' => 'fa fas fa-search-dollar', 'active' => request()->segment(2) == 'sell-payment-report']
                            );
                    ...
                    function ($sub) use ($enabled_modules) {
                        ...
                        if (auth()->user()->can('access_shipping')) {
                            $sub->url(
                                ...
                                ['icon' => 'fa fas fa-list', 'active' => request()->segment(1) == 'naturezas' && request()->segment(2) == null]
                            );
                        }
        ...
        $d = $moduleUtil->getModuleData('modifyAdminMenu');

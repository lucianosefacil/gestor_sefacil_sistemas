...
?>
<div class="row">
	<div class="pos-form-actions text-center">
		<div class="col-md-12">
			<?php if($is_mobile): ?>
			...
			<?php endif; ?>
			<button type="button" class="<?php if($is_mobile): ?> col-xs-6 <?php endif; ?> btn bg-yellow text-white btn-default btn-flat <?php if($pos_settings['disable_draft'] != 0): ?> hide <?php endif; ?>" id="pos-draft"><i class="fas fa-edit"></i> ORÇAMENTO</button>
           ...
           <!-- 
			...
			<button type="button" class="btn btn-default bg-info btn-flat <?php if($is_mobile): ?> col-xs-6 <?php endif; ?> text-white hide" id="pos-table"><i class="fas fa-table"></i> Salvar pedido</button>
			...
			<input type="hidden" name="final_total" 
										id="final_total_input" value=0>
			<span id="total_payable" style="font-size:48px;" class="text-success lead text-bold">0</span>
			&nbsp;&nbsp;
			...
			<?php endif; ?>
             ...
             <!-- AQUI OS BOTÕES DO LADO ESQUERDO -->          
            
			<?php if(empty($edit)): ?>
                    
            <button type="button" class="btn btn-primary btn-flat pull-right <?php if($is_mobile): ?> col-xs-6 <?php endif; ?>" data-toggle="modal" data-target="#recent_transactions_modal" id="recent-transactions"> <i style="font-size:18px;" class="fas fa-clock"></i> Transações recentes</button>
            
         
            
				<button type="button" class="btn btn-danger pull-right <?php if($is_mobile): ?> col-xs-6 <?php else: ?> btn-xs <?php endif; ?>" id="pos-cancel" style="padding:5px; margin-right:15px;"> <i style="font-size:18px;" class="fas fa-window-close"></i> CANCELAR</button>
			...
			<?php else: ?>
				...
				<button type="button" class="btn btn-danger btn-flat hide <?php if($is_mobile): ?> col-xs-6 <?php else: ?> btn-xs <?php endif; ?>" id="pos-delete"> <i class="fas fa-trash-alt"></i> <?php echo app('translator')->get('messages.delete'); ?></button>
			<?php endif; ?>
...
<?php echo $__env->make('sale_pos.partials.edit_quantidade_pdv', \Illuminate\Support\Arr::except(get_defined_vars(), ['__data', '__path']))->render(); ?>
<?php /**PATH /home/sefacilsistemasc/gestor.sefacilsistemas.com.br/resources/views/sale_pos/partials/pos_form_actions.blade.php ENDPATH**/ ?>
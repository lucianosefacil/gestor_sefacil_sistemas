			...
			<div class="row">
				<div class="form-group col-xs-12 <?php if(!auth()->user()->can('edit_product_price_from_sale_screen')): ?> hide <?php endif; ?>">
					<label>Valor unitário</label>
						<input type="text" name="products[<?php echo e($row_count, false); ?>][unit_price]" class="form-control pos_unit_price input_number mousetrap" value="<?php echo e(number_format(!empty($product->unit_price_before_discount) ? $product->unit_price_before_discount : $product->default_sell_price, 2, ',', '.'), false); ?>">
				</div>
				<?php
				...
				<div class="form-group col-xs-12 col-sm-6 <?php if(!$edit_discount): ?> hide <?php endif; ?>">
					<label><?php echo app('translator')->get('sale.discount_amount'); ?></label>
						<?php echo Form::text("products[$row_count][line_discount_amount]", number_format($discount_amount, 2, ',', '.'), ['class' => 'form-control input_number row_discount_amount']); ?>

				</div>

				<div class="form-group col-xs-12 col-sm-6">
					<label>Nº pedido</label>
						<?php echo Form::text("products[$row_count][n_pedido]", !empty($product->n_pedido) ? $product->n_pedido : '', ['class' => 'form-control']); ?>

				</div>
				<div class="form-group col-xs-12 col-sm-6">
					<label>Nº item</label>
						<?php echo Form::text("products[$row_count][n_item]", !empty($product->n_item) ? $product->n_item : '', ['class' => 'form-control']); ?>
				...
				<div class="form-group col-xs-12 <?php echo e($hide_tax, false); ?>">
					...
					<?php echo Form::hidden("products[$row_count][item_tax]", number_format($item_tax, 2, ',', '.'), ['class' => 'item_tax']); ?>
...
</div><?php /**PATH /home/sefacilsistemasc/gestor.sefacilsistemas.com.br/resources/views/sale_pos/partials/row_edit_product_price_modal.blade.php ENDPATH**/ ?>
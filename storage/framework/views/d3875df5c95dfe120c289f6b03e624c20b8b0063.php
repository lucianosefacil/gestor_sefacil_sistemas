...
<section class="content no-print">
    <?php $__env->startComponent('components.filters', ['title' => __('Filtrar Compras')]); ?>
    ...
    <?php echo $__env->make('purchase.partials.confirmacao_excluir', \Illuminate\Support\Arr::except(get_defined_vars(), ['__data', '__path']))->render(); ?>
...
<script>
    ...
    $(document).on('click', '.delete-purchase', function (e) {
        ...
        swal({
            title: 'Somente superadmin pode excluir',
            text: 'Para continuar, é necessário autorização de superadmin.',
            icon: 'warning',
            buttons: {
            cancel: { text: 'Fechar', visible: true },
            confirm: { text: 'Excluir', value: 'excluir' }
            },
            dangerMode: true
        }).then((value) => {
            if (value === 'excluir') {
            const $m = $('#modalSuperadminDelete');
            $m.find('[name="purchase_id"]').val(purchaseId);
            $m.find('[name="delete_url"]').val(deleteUrl);
            $m.modal('show');
            }
        });
    });

    // 2) Submeter modal para verificar superadmin
    $('#formSuperadminDelete').on('submit', function (e) {
        e.preventDefault();
        const $form = $(this);
        const deleteUrl = $form.find('[name="delete_url"]').val();
    $.post("<?php echo e(route('purchases.sudo-delete'), false); ?>", $form.serialize())
        .done(function () {
        // 3) Se ok, dispara o DELETE na URL original
        $.ajax({
            url: deleteUrl,
            type: 'POST',
            data: {
            ...
            $('#modalSuperadminDelete').modal('hide');
            swal('Excluída!', 'Compra excluída com sucesso.', 'success')
            .then(() => location.reload());
        }).fail(function (xhr) {
            swal('Erro', 'Falha ao excluir: ' + (xhr.responseJSON?.message || 'erro interno'), 'error');
        });
        })
        .fail(function (xhr) {
        swal('Não autorizado', (xhr.responseJSON?.message || 'Credenciais inválidas.'), 'error');
        });
    });
</script>
	
<?php $__env->stopSection(); ?>
<?php echo $__env->make('layouts.app', \Illuminate\Support\Arr::except(get_defined_vars(), ['__data', '__path']))->render(); ?><?php /**PATH /home/sefacilsistemasc/gestor.sefacilsistemas.com.br/resources/views/purchase/index.blade.php ENDPATH**/ ?>
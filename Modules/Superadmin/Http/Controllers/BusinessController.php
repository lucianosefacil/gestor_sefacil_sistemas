...
class BusinessController extends BaseController
{
    ...
    public function clearSubscriptions($business_id){
        try{
            Business::findOrFail($business_id)->subscriptions()->delete();
            $output = [
                'success' => 1,
                'msg' => "Ação realizada!"
            ];
    ...
    public function certificados(Request $request){
        ...
        if($data_inicial && $data_final){
            $businesses = Business::orderby('name', 'asc')->where('certificado', '!=', null)->get();
            foreach($businesses as $business){
                $expire = $this->getExpire($business, $data_inicial, $data_final);
        ...
        return view('superadmin::business.certificados', compact('data', 'data_inicial', 'data_final'));
    }

    private function getExpire($business, $data_inicial, $data_final){
        try{
            ...
            $vencimento = $publicKey->validTo->format('Y-m-d');
            if((strtotime($vencimento) > strtotime($data_inicial)) && (strtotime($vencimento) < strtotime($data_final))){
                return $vencimento;
            }
            return false;
        }catch(\Exception $e){
    ...
    public function search(Request $request)
    {
        ...
        if($request->type_search == 'razao_social'){
            $businesses->where('razao_social', 'like', "%$request->search%");
        }elseif($request->type_search == 'nome_fantasia'){
            $businesses->where('name', 'like', "%$request->search%");
        }elseif($request->type_search == 'cnpj'){
            $businesses->where('cnpj', 'like', "%$request->search%");
        }

        $businesses = $businesses->paginate(21);

        $business_id = request()->session()->get('user.business_id');
        return view('superadmin::business.index')
        ->with(compact('businesses', 'business_id'))
        ->with('type_search', $request->type_search)

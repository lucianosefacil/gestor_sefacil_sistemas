...
class JobSheetController extends Controller
{
    ...
    public function edit($id)
    {
        ...
        return view('repair::job_sheet.edit')
            ->with(compact('job_sheet', 'repair_statuses', 'device_models', 'brands', 'devices', 'default_status', 'technecians', 'types', 'customer_groups', 'repair_settings', 'tipo', 'usuario'))
            ->with('cities', $this->prepareCities())
            ->with('estados', $this->prepareEstados())
            ->with('paises', $this->preparePaises());;
    }
    ...
    public function addParts($id)
    {
        ...
        // se ja houver produtos 
        $location_id = $job_sheet->location_id;
        $location_printer_type = BusinessLocation::find($location_id)->receipt_printer_type;

        $sell_details = ProdutoOs::
        join(
            'products AS p',
            'produto_os.produto_id',
            '=',
            'p.id'
        )
        ->join(
        ...
        ->where('produto_os.produto_id', $id)
        ->with(['warranties'])
        ...
        ->select(
            ...
            'produto_os.id',
            'produto_os.quantidade as quantity_ordered',
            'units.id as unit_id',
            DB::raw('vld.qty_available + produto_os.quantidade AS qty_available')
        )
        ->get();
        ...
        return view('repair::job_sheet.add_parts')
            ...
            'default_location', 'business_details', 'walk_in_customer', 'sell_details', 'taxes', 'edit_price', 'edit_discount', 'warranties'));
    }
    ...
    public function saveParts(Request $request, $id)
    {
        ...
        try {
            ...
            // salvar produtos_os e servicos_os
            $job_sheet->produtos()->delete();

            foreach ($request->products as $value) {
                    $produtos = [
                        'produto_id' => $value['product_id'],
                        'variation_id' => $value['variation_id'],
                        'ordem_servico_id' => $job_sheet->id,
                        'quantidade' => $value['quantity'],
                        'valor_unitario' => $value['unit_price_inc_tax'],
                        'sub_total' => $this->convert_num($value['unit_price_inc_tax']) * $this->num_uf($value['quantity'])
                    ]; 
                    // dd($produtos);
                    ProdutoOs::create($produtos);
                }
    ...
    public function convert_num($number)
    {

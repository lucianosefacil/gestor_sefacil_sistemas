...
class CreateCtesTable extends Migration
{
    ...
    public function up()
    {
        Schema::create('ctes', function (Blueprint $table) {
            ...
            $table->integer('recebedor_id')->nullable()->unsigned();
            $table->foreign('recebedor_id')->references('id')
            ->on('contacts');

            $table->integer('expedidor_id')->nullable()->unsigned();
            ...
            // alter table ctes add column globalizado integer;
            // alter table ctes add column cst varchar(3);
            // alter table ctes add column perc_icms decimal(5, 2);

...
<section class="content">
  ...
  <div class="row">
    <div class="col-md-12">
      ...
      <div class="col-md-12">
        <h4>Número NFe de Entrada: <strong>{{$transaction->numero_nfe}}</strong></h4>
  ...
  <div class="row" id="action" style="display: none">
    ...
    <script type="text/javascript">
      ...
      $('#cancelar').click(() => {
        ...
        swal({
          text: 'Cancelamento de NFe '+numero_nfe+'.',
          content: "input",
          button: {
            text: "Cancelar!",
            closeModal: false,
          ...
          if (!v) swal("Erro!", "Informe um motivo para Cancelamento!", "error");
            ...
            ({
              ...
              success: function(e){
                ...
                if (e.success) {
                  const motivo = e?.retEvento?.infEvento?.xMotivo ?? e?.mensagem ?? 'Cancelamento realizado com sucesso.';
                  ...
                  // sucesso HTTP mas erro lógico (ex: rejeição da SEFAZ)
                  const motivo = e?.retEvento?.infEvento?.xMotivo ?? e?.mensagem ?? "Erro ao cancelar.";
                  // const msg = e?.mensagem ?? "Erro ao cancelar.";
                  swal("Erro ao cancelar", motivo, "error");
                }


              }, error: function(e){
                console.log("Error response:", e);
                let msg = "Erro inesperado ao comunicar com o servidor.";

                try {
                  let res = e.responseJSON;
                  if (res?.mensagem) {
                    msg = res.mensagem;
                  } else if (res?.data?.retEvento?.infEvento?.xMotivo) {
                    msg = res.data.retEvento.infEvento.xMotivo;
                  } else if (res) {
                    msg = JSON.stringify(res);
                  }
                } catch (err) {
                  console.log("Erro ao interpretar erro:", err);
                }
          ...
          if (!v) swal("Erro!", "Informe a correção!", "error");
            ...
            ({
              ...
              success: function(e){
                ...
                .then(() => {
                  window.open(path + '/nfeEntrada/imprimirCorrecao/'+id)
                  location.reload()
                });

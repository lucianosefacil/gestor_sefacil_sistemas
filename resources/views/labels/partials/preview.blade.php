...
@foreach($product_details as $details)
	@while($details['qty'] > 0)
		...
		<div style="height:{{$barcode_details->height}}in !important; line-height:in; width:{{$barcode_details->width*0.97}}in !important; display: inline-block; @if(!$is_new_row) @endif @if(!$first_row)margin-top:{{$barcode_details->row_distance}}in !important; @endif" class="sticker-border text-center">
		<div style="display:inline-block;vertical-align:middle;line-height:25px !important;">
			{{-- Business Name --}}
			@if(!empty($print['business_name']))
				<b style="font-size:25px" class="text-uppercase">{{$business_name}}</b>
			@endif

			{{-- Product Name --}}
			@if(!empty($print['name']))
				<span style="display: block !important; font-size:28; margin-bottom:1px;">
			...
			@if(!empty($print['variations']) && $details['details']->is_dummy != 1)
				<span style="display: block !important; font-size:30px">
			...
			@if(!empty($print['price']))
				<b style="font-size:28px;">@lang('lang_v1.price'):</b>
			...
			<br>
			{{-- Barcode --}}
			<img class="center" style="max-width:93%; margin-left:-1px; !important;height" src="data:image/png;base64,{{DNS1D::getBarcodePNG($details['details']->sub_sku, $details['details']->barcode_type, 3,30,array(39, 48, 54), true)}}">

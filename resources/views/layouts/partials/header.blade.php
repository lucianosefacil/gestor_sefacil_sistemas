...
<header class="main-header no-print">
    ...
    <nav class="navbar navbar-static-top" role="navigation">
        ...
        <div class="navbar-custom-menu">
            ...
            @if (auth()->user()->business->certificate_request_status == 'AGUARDANDO')
                <div class="m-8 pull-left mt-15 hidden-xs" style="text-align: center; ">
                    <strong style="color: #fff;">Certificado:</strong>
                    <span style="color: #fff;">
                        Pedido em andamento
                    </span>
                    <a href="{{ auth()->user()->business->redirecionarPedido()['redirectTo'] }}" target="_blank"
                        class="btn btn-warning btn-xs" style="color: #fff;">
                        Acompanhar
                    </a>
                </div>
            @elseif(auth()->user()->business->certificate_request_status == 'PAGO')
                <div class="m-8 pull-left mt-15 hidden-xs" style="text-align: center; ">
                    <strong style="color: #fff;">Certificado:</strong>
                    <span style="color: #fff;">
                        Pedido em andamento
            ...
            @if ($youtube)
                @can('sell.create')
                    <a href="{{ $youtube->url_from_youtube }}" title="Youtube" data-toggle="tooltip" target="_blank"
                        data-placement="bottom"
                        style="background-color: {{ $youtube->background_color }};border-color: {{ $youtube->background_color }} "

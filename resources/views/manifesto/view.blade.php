...
<section class="content">
	...
	{!! Form::open(['url' => '/manifesto/save', 'method' => 'post', 'id' => 'add_purchase_form', 'files' => true ]) !!}
	...
	<div class="col-sm-3">
		<div class="form-group">
			...
			{!! Form::select('location_id', $business_locations, !$location_id ? $default_location->id : $location_id, ['class' => 'form-control select2', 'placeholder' => __('messages.please_select'), 'required']); !!}
		</div>
	</div>
	...
	<div class="row">
		<div class="col-sm-12">
			<div class="form-group">
				...
				<p class="text-danger">*Este fornecedor não esta cadastrado no sistema, será cadastrado após salvar!</p>
				@endif
				<div class="row">
					<div class="col-sm-6">
					...
					<div class="col-sm-6">
						...
						<p class="text-danger" style="font-size: 20px; margin-top: 10px;">>>Cadastre o fornecedor para incluir os produtos!</p>
						@endif -->
					</div>
												...
												<tbody id="itens">
													...
													<tr>
														<!-- <form @if(!$i['produtoNovo']) action="/manifesto/atribuirEstoque" @else action="/manifesto/cadProd" @endif method="get"> -->
															...
															<input type="hidden" value="{{$dadosNf['chave']}}" name="chave">

															<!-- td style="width: 80px;">
																@if($i['produtoNovo'])
																<button @if($dadosNf['novoFornecedor']) disabled @endif class="btn btn-success" type="submit">
																	Adicionar
																</button>
																@endif

																@if(!$i['produtoNovo'] && !$i['tp'])
																<button @if($dadosNf['novoFornecedor']) disabled @endif class="btn btn-danger" type="submit">
																	Atribuir estoque
																</button>
																@endif

																@if($i['tp'])
																<a @if($dadosNf['novoFornecedor']) disabled @endif class="btn btn-danger" disabled >
																	Atribuir estoque
																</a>
																@endif
															</td> -->
		...
		<div class="row">
			<div class="col-sm-12">
				<a href="/manifesto/baixarXml/{{$chave}}" class="btn btn-primary btn-flat">Baixar XML</a>

				@if(!$salvoEmCompras)
				<button type="submit" class="btn btn-success pull-right btn-flat">Salvar Compra</button>
				@else
				<button type="button" class="btn btn-success pull-right btn-flat" disabled>
					Xml já incluido em vendas
				</button>

...
@section('content')
    ...
    <section class="content">
        @component('components.widget', [
            'class' => 'box-primary',
            'title' => ($item ? 'Editar' : 'Cadastrar') . ' Configuração NFSe',
        ])

            <form method="post" action="{{ $item ? route('nfse-config.update', [$item->id]) : route('nfse-config.store') }}"
                    ...
                    <div class="col-sm-2">
                        <div class="form-group">
                            ...
                            <input id="documento" type="text" name="documento"
                                class="form-control cpf_cnpj @error('documento') is-invalid @enderror"
                    ...
                    <div class="col-sm-2">
                        <div class="form-group">
                            ...
                            <input id="ie" type="text" name="ie"
                                class="form-control @error('ie') is-invalid @enderror" value="{{ old('ie', $item->ie ?? '') }}">
                        </div>
                    </div>
                </div>
                ...
                <div class="row">
                    ...
                    <div class="col-sm-2">
                        <div class="form-group">
                            ...
                            <input type="text" name="codigo_tributacao_municipio"
                                class="form-control @error('codigo_tributacao_municipio') is-invalid @enderror"
                    ...
                    <div class="col-sm-2">
                        <div class="form-group">
                            <label>Número</label>
                            <input id="numero" type="tel" name="numero"
                    ...
                    <div class="col-sm-5">
                        <div class="form-group">
                            <label>Cidade</label>
                            <select class="form-control select2 @error('cidade_id') is-invalid @enderror" id="kt_select2_1"
                                name="cidade_id" required>
                                <option value="">Selecione a cidade</option>
                                @foreach ($cidades as $c)
                                    <option value="{{ $c->id }}" @if (old('cidade_id', $item->cidade_id ?? null) == $c->id) selected @endif>
                                        {{ $c->nome }} ({{ $c->uf }})
                                    </option>
                                @endforeach
                            </select>
                    ...
                    <div class="col-sm-6">
                        <div class="form-group">
                            ...
                            @if (isset($item) && $item->token)
                                <input class="form-control" type="password" name="token" value="{{ $item->token ?? '' }}"
                                    @if (!$item)  @endif disabled>
                            @else
                        ...
                        @if (!empty($item) && !empty($item->token))
                            <div class="btn-group" style="margin-top:10px;">
                                ...
                                {{-- Gerar NOVO token --}}
                                {{-- <form action="{{ route('nfse-config.new-token') }}" method="get" style="display:inline-block; margin-left:5px;">
                <button type="submit" class="btn btn-warning btn-sm"
                        onclick="return confirm('Gerar um NOVO token? O token atual deixará de funcionar.');">
                  <i class="la la-refresh"></i> Gerar novo token
                </button>
              </form> --}}
                            </div>
                        @else
                            <a href="{{ route('nfse-config.new-token') }}" class="btn btn-warning btn-sm"
                                onclick="return confirm('Gerar um NOVO token? O token atual deixará de funcionar.');">
                                <i class="la la-refresh"></i> Gerar token inicial
                            </a>
                        @endif
...
@section('javascript')
    <script>
        ...
        $('#documento').on('blur', function() {
            ...
            var url = '{{ url('contacts/buscar-cnpj') }}/' + doc;
            $.get(url)
                .done(function(data) {
                    console.log(data);
                    ...
                    if (est.cidade && est.cidade.ibge_id) {
                        definirCidadePorIbge(est.cidade.ibge_id, est.estado ? est.estado.sigla : null);
                    }

                }).fail(function(err) {
                    swal('Erro', err.responseJSON.erro, 'error');
                })
        });
        
        function definirCidadePorIbge(ibge, uf) {
            $.get('{{ route('nfe.findCidade') }}', { ibge: ibge, uf: uf })

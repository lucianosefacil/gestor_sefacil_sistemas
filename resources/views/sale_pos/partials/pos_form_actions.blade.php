...
<div class="row">
	<div class="pos-form-actions text-center">
		<div class="col-md-12">
			...
			@endif
			<button type="button" class="@if($is_mobile) col-xs-6 @endif btn bg-yellow text-white btn-default btn-flat @if($pos_settings['disable_draft'] != 0) hide @endif" id="pos-draft"><i class="fas fa-edit"></i> ORÇAMENTO</button>
           ...
           <!-- 
			...
			<button type="button" class="btn btn-default bg-info btn-flat @if($is_mobile) col-xs-6 @endif text-white hide" id="pos-table"><i class="fas fa-table"></i> Salvar pedido</button>
			...
			<button type="button" class="btn bg-navy btn-default @if(!$is_mobile) @endif btn-flat no-print @if($pos_settings['disable_pay_checkout'] != 0) hide @endif @if($is_mobile) col-xs-6 @endif" id="pos-finalize" title="@lang('lang_v1.tooltip_checkout_multi_pay')"><i class="fas fa-money-check-alt" aria-hidden="true"></i> PAGAMENTO </button>
            ...
            <button type="button" class="btn btn-primary btn-flat pull-right @if($is_mobile) col-xs-6 @endif" data-toggle="modal" data-target="#recent_transactions_modal" id="recent-transactions"> <i style="font-size:18px;" class="fas fa-clock"></i> Transações recentes</button>
            
         
            
				<button type="button" class="btn btn-danger pull-right @if($is_mobile) col-xs-6 @else btn-xs @endif" id="pos-cancel" style="padding:5px; margin-right:15px;"> <i style="font-size:18px;" class="fas fa-window-close"></i> CANCELAR</button>
...
@include('sale_pos.partials.edit_quantidade_pdv')

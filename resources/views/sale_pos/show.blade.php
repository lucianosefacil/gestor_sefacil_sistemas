  ...
  <div class="modal-content">
    ...
    <h4 class="modal-title" id="modalTitle"> 
        @if($sell->status == 'pending')
          @if($sell->is_locacao == 1)
            @lang('Detalhes da')
              {{ __('Locação') }} 
              (<b>@lang('Numero'): {{ $sell->invoice_no }})</b> 
              <br>   
          @else
            @lang('Detalhes do')
            {{ __('Orçamento') }} 
...
<div class="modal-body">
    ...
    <div class="row">
      <div class="col-sm-4">
          @if($sell->status == 'pending')
              @if($sell->is_locacao == 1)
              <b>@lang{{'Status'}}: {{ 'Locação' }}</b> 
              <br>
              @else
              <b>@lang{{'Status'}}: {{ 'Orçamento' }}</b> 
              <br>
              @endif
              <b>@lang{{'Numero'}}: {{ $sell->invoice_no }}</b>
              <br>
            <b>@if(empty($sell->payment_status)){{('Pagamento: '. 'Em Aberto') }}</b> 
          ...
          @else
            @if($sell->is_locacao == 1)
              ...
              <br>   
            @else
              <b>@lang {{'Status:'}} {{'Venda'}}</b> 
              <br>   
            @endif    
              <b>@lang {{'Numero da Venda:'}} {{ $sell->invoice_no }}</b> 
          @endif
              <br>     
              
            @if(!empty($sell->payment_status))
              <b>{{ __('sale.payment_status') }}:</b>
              <b>{{ __('lang_v1.' . $sell->payment_status) }}</b>
              <br>
            @endif
  ...
  <div class="modal-footer">
    ...
    <a href="#" class="print-invoice btn btn-primary" data-href="{{route('sell.printInvoice', [$sell->id])}}"><i class="fa fa-print" aria-hidden="true"></i> Impressão</a>

    {{-- @isset($servicos)
    <a href="{{route('sell.printVenda', [$sell->id])}}" class="btn btn-primary"><i class="fa fa-print" aria-hidden="true"></i> Impressão 2</a>
    @endisset   --}}

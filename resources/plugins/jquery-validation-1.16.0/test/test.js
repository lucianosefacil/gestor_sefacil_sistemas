...
QUnit.test( "defaultMessage(), empty title is ignored", function( assert ) {
	...
	assert.equal( v.defaultMessage( $( "#username" )[ 0 ], { method: "required", parameters: true } ), "Campo obrigatório." );

	// Using the old way when we pass the name of a method as the second parameters.
	assert.equal( v.defaultMessage( $( "#username" )[ 0 ], "required" ), "Campo obrigatório." );
} );
...
QUnit.test( "works on contenteditable fields", function( assert ) {
	...
	assert.hasError( $( "#contenteditableRequiredInvalid" ), "Campo obrigatório." );
